...
export interface ExperienceItem { id: string; company: string; position: string; startDate: string; endDate: string; description: string }
...
export default function ResumePreview({ cv }: { cv: CVDataPreview }) {
  ...
  return (
    <div
      ...
      style={{ borderTopWidth: 15, borderTopColor: cv.themeColor || '#000000', borderTopStyle: 'solid', fontSize: `${fontSize}pt` }}
    >
      <div className="mb-2">
        <div className="flex justify-between items-start border-b border-gray-200 pb-3">
          <div>
            <h1 className="font-bold text-gray-900 leading-tight" style={{ fontSize: `${nameSize}pt` }}>
              <div>{firstLine}</div>
              {nextLine && <div>{nextLine}</div>}
            </h1>
            {cv.personalInfo.jobTitle && (
              <p className="text-gray-800 font-medium mt-1" style={{ fontSize: `${titleSize}pt` }}>{cv.personalInfo.jobTitle}</p>
            )}
          </div>
          <div className="text-right text-sm text-gray-600 space-y-0.5">
            {cv.personalInfo.email && (
              <p>
                {cv.linkifyContacts ? (
                  <a href={`mailto:${cv.personalInfo.email}`} className="underline">{cv.personalInfo.email}</a>
                ) : (
                  cv.personalInfo.email
                )}
              </p>
            ...
            {cv.personalInfo.phone && (
              <p>
                {cv.linkifyContacts ? (
                  <a href={`tel:${cv.personalInfo.phone}`} className="underline">{cv.personalInfo.phone}</a>
                ) : (
                  cv.personalInfo.phone
                )}
              </p>
            ...
            {cv.personalInfo.linkedin && (
              <p>
                {cv.linkifyContacts ? (
                  <a href={cv.personalInfo.linkedin} target="_blank" className="underline">LinkedIn</a>
                ) : (
                  cv.personalInfo.linkedin
                )}
              </p>
            ...
            {cv.personalInfo.github && (
              <p>
                {cv.linkifyContacts ? (
                  <a href={cv.personalInfo.github} target="_blank" className="underline">GitHub</a>
                ) : (
                  cv.personalInfo.github
                )}
              </p>
      ...
      {cv.summary && (
        <div className="mb-3">
          <h2 className="text-lg font-semibold text-gray-900 mb-1">Professional Summary</h2>
        ...
        <div className="mb-2">
          ...
          <div className="flex flex-wrap gap-2">
            {cv.skills.map((skill, idx) => (
              <span key={idx} className="text-xs bg-gray-100 text-gray-800 px-3 py-1 rounded-full">{skill}</span>
            ))}
          </div>
        </div>
      )}
