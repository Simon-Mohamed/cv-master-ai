...
export default function JobSearchPage() {
  ...
  const toggleSaveJob = (jobId: string) => {
    const updated = savedJobs.includes(jobId) ? savedJobs.filter((id) => id !== jobId) : [...savedJobs, jobId]
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <p className="text-muted-foreground mb-8">Find jobs matched to your skills and experience</p>

          {/* Filters */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Search by Title or Company</label>
              <input
                type="text"
                value={searchTerm}
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">Filter by Location</label>
              <input
                type="text"
          ...
          <div className="space-y-4">
            {filteredJobs.length > 0 ? (
              filteredJobs.map((job) => (
                <div key={job.id} className="border border-border rounded-lg p-6 hover:shadow-md transition">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <button
                        ...
                        className={`text-2xl ${savedJobs.includes(job.id) ? "text-accent" : "text-gray-300"}`}
                      >
                        ♥
                      </button>
                    </div>
